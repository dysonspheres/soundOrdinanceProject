...
# there are supposed to be HELLA outliers so idk where they are
...
df_avg_td_mon <- df_times %>% group_by(in_mon) %>% summarize(avg_time_diff = mean(time_diff))

ggplot(df_avg_td_mon, aes(x=in_mon, y=avg_time_diff)) + geom_bar(stat = 'identity')

#scatter plot of in month vs issued month -- will this show counts of when things get applied, and when things get issued?

# time difference per SEASON, {theme by color?}

...
convert_time <- function(time) {
  ...
  if (ampm == "am") {
    ...
    ampm <- ""  # In case no AM/PM is found
  }
  ...
  # Match the different time formats and return the appropriate numeric value
  if (grepl("midnight|12:00a|12 am|12a", time)) {
    return(0)
  } else if (grepl("noon|12:00p|12 pm|12p", time)) {
    return(12)
  } else if (grepl("am", time)) {
  ...
  } else if (grepl("pm", time)) {
    time <- sub("pm", "", time)  # Remove 'pm'
    time <- trimws(time)  # Trim extra spaces
    ...
    return(time)  # If time format is unrecognized, return the original string
  }
...
# remove tghngs with the wrong time
# put time vector in funcyion

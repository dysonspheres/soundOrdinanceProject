...
# removing anything that is not a number for this section
df <- df %>% mutate(CAPACITY = tolower(CAPACITY)) %>% 
  mutate(CAPACITY = ifelse(str_detect(CAPACITY, "load card"), NA,CAPACITY )) %>% 
  mutate(CAPACITY = ifelse(str_detect(CAPACITY, "door"), NA,CAPACITY ))  %>% 
  mutate(CAPACITY = ifelse(str_detect(CAPACITY, "126 outside"), "126",CAPACITY ))  %>% 
  mutate(CAPACITY = ifelse(str_detect(CAPACITY, "211 upstairs"), "451",CAPACITY )) %>% 
  mutate(CAPACITY = ifelse(str_detect(CAPACITY, "133 outside"), "133",CAPACITY )) %>% mutate(CAPACITY = as.numeric(CAPACITY))
...
df <- df %>%
  mutate(EXISTING_ZONING = case_when(
    ...
    EXISTING_ZONING == "SF-6 - TOWNHOME, CONDO" ~ "SF-6",
    EXISTING_ZONING == "R&D RESEARCH & DEVELOPMENT" ~ "R&D",
    EXISTING_ZONING == "PUD - PLANNED UNIT DEVELOPMENT" ~ "PUD",
    EXISTING_ZONING == "P-NP Public-Neighborhood Plan Combining district" ~ "P-NP",
    EXISTING_ZONING == "P - PUBLIC" ~ "P",
    EXISTING_ZONING == "NO - NEIGHBOORHOOD OFFICE" ~ "NO",
    EXISTING_ZONING == "MI - MAJOR INDUSTRY" ~ "MI",
    EXISTING_ZONING == "MF-5 - MULTI FAMILY HIGH DENSITY" ~ "MF-5",
    EXISTING_ZONING == "MF-4 - MULTI FAMILY MODERATE" ~ "MF-4",
    EXISTING_ZONING == "LR - NEIGHBOORHOOD COMMERCIAL" ~ "LR",
    EXISTING_ZONING == "LO - LIMITED OFFICE" ~ "LO",
    EXISTING_ZONING == "LI - LIMITED INDUSTRIAL SERVICES" ~ "LI",
    EXISTING_ZONING == "IRR - INTERIM RURAL RESIDENCE" ~ "IRR",
    EXISTING_ZONING == "IP - INDUSTRIAL PARK" ~ "IP",
    EXISTING_ZONING == "GO - GENERAL OFFICE" ~ "GO",
    EXISTING_ZONING == "DR - DEVELOPMENT RESERVE" ~ "DR",
    EXISTING_ZONING == "CR - COMMERCIAL REC" ~ "CR",
    EXISTING_ZONING == "cbd-cure" ~ "CBD-CURE",
...
# Nneoma cleaning Issue dates and In dates
...
```{r}
# THIS IS FOR START HRS NOT NEAR HOME
# next time I'll replace with median instead of mean (bc of SD)
...
ggplot(df_clean_start) + 
  ...
  labs(title = "Boxplot of Start & End Times for Start Hours Not Near Home", y = "Hour", x="Starts and Ends")+
  theme_minimal()
...
ggplot(df_clean_start, aes(x = factor(START_HOURS_NOT_NEAR_HOME), fill = START_HOURS_NOT_NEAR_HOME)) +
  ...
  theme_minimal() +
  labs(title='Start Times for Start Hrs NOT Near Home', x = "Start Hour", y = "Count", fill = "Hour of Day")+
  theme_minimal()
...
ggplot(df_clean_end, aes(x = factor(END_HOURS_NOT_NEAR_HOME), fill = END_HOURS_NOT_NEAR_HOME)) +
  ...
  scale_fill_gradient2(low = "darkblue", mid = "yellow", high = "darkblue", midpoint = 12) +
  theme_minimal() +
  labs(title='End Times for End Hrs NOT Near Home', x = "End Hour", y = "Count", fill = "Hour of Day") +
  theme_minimal()
...
ggplot(df_times, aes(x = time_diff)) + 
  geom_histogram(binwidth = 30, fill = "#76B5C5", color = "black", alpha = 0.8, boundary = 0) + 
  labs(
    x = "Time Difference (Days)", 
    y = "Frequency", 
    title = "Distribution of Time Differences (Application to Issuance)"
  ) +
  scale_x_continuous(
    limits = c(0, 240),
...
ggplot(df_times, aes(x = time_diff)) + 
  ...
  theme(
    ...
    axis.title = element_text(face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1),
    aspect.ratio = 0.4  # adjust this value to make it wider
  )

# df_times %>% summarize(mean(time_diff))
print('The average time applicants wait for their permits to be issued is 19.6 days')

# I want to see the median time difference bassed on the date applied by MONTH  -- changed from mean

df_times <- df_times %>% mutate(IN_DATE = as.Date(IN_DATE),
    ISSUE_DATE = as.Date(ISSUE_DATE),in_mon = format(IN_DATE, "%B"), issue_mon = format(ISSUE_DATE, "%B"), in_year = format(IN_DATE, "%Y" ),  issue_year = format(ISSUE_DATE, "%Y")) %>% 
    mutate(in_mon = factor(in_mon, levels = month.name), issue_mon = factor(issue_mon, levels = month.name))
df_avg_td_mon <- df_times %>% group_by(in_mon) %>% summarize(avg_time_diff = mean(time_diff), med_time_diff = median(time_diff), sd_time_diff = sd(time_diff))
...
labs(title = 'Months Where People Apply for a Permit', x='Application Months', y='Number of Permit Applications') +
  theme_minimal()
...
labs(title='Months Where Permist get Issued', x='Issue Months', y='Number of Permit Applications') +
  theme_minimal()
  
# --------------------------------------------------------------------------------------

df_times <- df_times %>% mutate(ZIPCODE = as.character(ZIPCODE))
df_zips <- df_times %>% group_by(ZIPCODE) %>% summarize(avg_time_diff = mean(time_diff, na.rm = TRUE), med_time_diff = median(time_diff), sd_time_diff = sd(time_diff, na.rm = TRUE), counts = n())

# Average time by Zipcode
ggplot(df_zips, aes(x = ZIPCODE, y = avg_time_diff)) + geom_bar(stat = 'identity') + 
geom_errorbar(
    aes(ymin = avg_time_diff - sd_time_diff, ymax = avg_time_diff + sd_time_diff), 
    width = 0.2,  # Adjust error bar width
    color = "red"  # Error bars in red
...
# zipcode boxplot
ggplot(df_times) + 
  geom_boxplot(aes(x = ZIPCODE, y = time_diff), fill = "blue", alpha = 0.6)+ theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7))

# number of applications per zipcode
ggplot(df_times, aes(x=ZIPCODE)) + geom_bar(stat = 'count') + labs(title='Number of Applications Per Zip Code')+
  theme_minimal()

avg_dec_zip <- df_times %>% filter(DECIBEL_LEVEL < 8000) %>% group_by(ZIPCODE) %>% summarize(avg_dec = mean(DECIBEL_LEVEL, na.rm = TRUE), sd_dec = sd(DECIBEL_LEVEL, na.rm = TRUE))
ggplot(avg_dec_zip, aes(x=ZIPCODE, y=avg_dec)) + geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) + labs(title='Average Decibel Level per Zipcode', y='Average Decibel')
  theme_minimal()

avg_dec_sfg <- df_times %>% filter(DECIBEL_LEVEL < 8000) %>% group_by(DECIBEL_LEVEL) %>% summarize(avg_cap = mean(CAPACITY, na.rm = TRUE), sd_cap = sd(CAPACITY, na.rm = TRUE))
ggplot(avg_dec_sfg, aes(x=DECIBEL_LEVEL, y=avg_cap)) + geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) + geom_errorbar(
    aes(ymin = avg_cap - sd_cap, ymax = avg_cap + sd_cap), 
    width = 0.2,  # Adjust error bar width
    color = "red"  # Error bars in red
  ) + labs(title='Average Capacity at Each Decibel Level', x='Decibel Levels', y='Average Capacity') +
  theme_minimal()

# time to get permit by capacity
df_cap_NO_OUTLIERS <- df_times %>% filter(CAPACITY < 10000)

ggplot(df_cap_NO_OUTLIERS, aes(x = CAPACITY, y = time_diff)) + 
  geom_point(na.rm = TRUE) + geom_smooth(method='lm') + labs(title='Capacity vs Average Time to Get Permit Issues', y='Average Time Difference (days)')  +
  theme_minimal()


# Now we are doing average time differences with decibel level
df_dec <- df_times %>% filter(DECIBEL_LEVEL < 8000) %>% group_by(DECIBEL_LEVEL) %>% summarize(avg_time_diff = mean(time_diff), sd_time_diff = sd(time_diff))
ggplot(df_dec, aes(x = factor(DECIBEL_LEVEL), y = avg_time_diff)) + 
  geom_bar(stat = 'identity') + 
  geom_errorbar(
    aes(ymin = avg_time_diff - sd_time_diff, ymax = avg_time_diff + sd_time_diff), 
    width = 0.2,  # Adjust error bar width
    color = "red"  # Error bars in red
  ) + labs(title='Average Time Before Receiving Permit by Decible Level', x='Decibel Levels', y='Average Time to Get Permit Issued (days)')  +
  theme_minimal()

# approval time per subtype
df_sub <- df_times %>% group_by(SUB_TYPE) %>% summarize(avg_time_diff = mean(time_diff), sd_time_diff = sd(time_diff))
'''
ggplot(df_sub, aes(x=SUB_TYPE, y=avg_time_diff, fill=SUB_TYPE)) + geom_bar(stat = 'identity') + 
  geom_errorbar(
    aes(ymin = avg_time_diff - sd_time_diff, ymax = avg_time_diff + sd_time_diff), 
    width = 0.2,  # Adjust error bar width
    color = "red"  # Error bars in red
  ) + labs(title=Average Time Before Receiving Permit by Permit Subtype, x='Permit Subtype', y='Average Time to Get Permit Issued (days)')  +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10))
'''

ggplot(df_sub, aes(x=SUB_TYPE, y=avg_time_diff, fill=SUB_TYPE)) + 
  geom_bar(stat = 'identity', width = 0.6) +  # Reduce bar width for better spacing
  geom_errorbar(
    aes(ymin = pmax(avg_time_diff - sd_time_diff, 0), ymax = avg_time_diff + sd_time_diff), 
    width = 0.2,  
    color = "red" ) + 
  labs(title='Average Time Before Receiving Permit by Permit Subtype', 
       x='Permit Subtype', 
       y='Average Time to Get Permit Issued (days)') +
  theme_minimal() +
  theme( 
    axis.text.x = element_text(angle = 25, hjust = 1, vjust = 1, size = 10),  # Less rotation
    plot.title = element_text(size = 14, face = "bold", margin=margin(b=10)),  # Reduce white space
    legend.position = "none",
    plot.margin = margin(10, 10, 10, 10)  # Tighten plot spacing
  ) +
  scale_y_continuous(limits = c(0, max(df_sub$avg_time_diff + df_sub$sd_time_diff, na.rm = TRUE) * 1.1)) +  
  coord_cartesian(ylim = c(0, max(df_sub$avg_time_diff + df_sub$sd_time_diff, na.rm = TRUE) * 1.1)) + 
  theme(aspect.ratio = 0.5)  # Make plot shorter

'''
# DECIBELS ACROSS THE YEARS
def_dec_yr <- df_times %>% filter(DECIBEL_LEVEL < 8000) %>% group_by(DECIBEL_LEVEL)
def_dec_yr %>% glimpse()
ggplot(def_dec_yr, aes(x=DECIBEL_LEVEL)) + geom_bar(stat = 'count') + labs(title='Permits with Recorded Decibels', x='Decibel Levels') + facet_wrap(~ in_year) + theme_minimal()

unique(df$SUB_TYPE)
'''

```
# Greg's Exploratory Graphing
```{r}
# GREG VISUALIZATIONS
...
```
# Maadhav's Exploratory Graphing
```{r}
#MAADHAV VISUALIZATIONS

#Coordinate map of Lat vs Long by decibel levels
library(ggplot2)
library(ggmap)
library(tidyr)

decibel_df <- df %>%
  filter(!is.na(LATITUDE) & !is.na(LONGITUDE) & !is.na(DECIBEL_LEVEL) & DECIBEL_LEVEL < 194)

lat <- decibel_df$LATITUDE
long <- decibel_df$LONGITUDE
decibel_levels <- decibel_df$DECIBEL_LEVEL

p0 <- ggplot(decibel_df, aes(x = long, y = lat, color = decibel_levels)) +
  geom_point(size = 3, alpha = 0.7) +
  scale_color_viridis_c(option = "plasma", limits = c(min(decibel_df$DECIBEL_LEVEL), max(decibel_df$DECIBEL_LEVEL)), oob = scales::squish) +
  labs(title = "Most Noisy Locations", color = "Decibels") +
  theme_minimal()

print(p0)


#Street Address vs Capacity
clean_data <- df %>%
  filter(!is.na(STREET_ADDRESS) & !is.na(CAPACITY))

clean_data <- clean_data %>%
  mutate(CAPACITY = as.character(CAPACITY)) %>%
  filter(grepl("^[0-9]+$", CAPACITY))

clean_data <- clean_data %>%
  mutate(CAPACITY = as.numeric(CAPACITY))
N <- 100
top_capacity <- clean_data %>%
  arrange(desc(CAPACITY)) %>%
  slice_head(n = N)

p1 <- ggplot(top_capacity, aes(x = reorder(STREET_ADDRESS, CAPACITY), y = CAPACITY)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 30 Street Addresses by Capacity", x = "Street Address", y = "Capacity") +
  theme_minimal()

print(p1)

top_capacity_counts <- clean_data %>%
  arrange(desc(CAPACITY)) %>%
  slice_head(n = N) %>%
  count(STREET_ADDRESS, sort = TRUE)

p2 <- ggplot(top_capacity_counts, aes(x = reorder(STREET_ADDRESS, n), y = n)) +
  geom_col(fill = "steelblue") +
  coord_flip() +  # Rotate for readability
  labs(title = paste("Count of Street Addresses in Top", N, "Capacities"),
       x = "Street Address", y = "Count") +
  theme_minimal()

print(p2)


clean_data <- df %>% filter(!is.na(STREET_ADDRESS) & !is.na(DECIBEL_LEVEL) & DECIBEL_LEVEL < 194)

N <- 100
top_decibels <- clean_data %>%
  arrange(desc(DECIBEL_LEVEL)) %>%
  slice_head(n = N)

p3 <- ggplot(top_decibels, aes(x = reorder(STREET_ADDRESS, DECIBEL_LEVEL), y = DECIBEL_LEVEL)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 100 Street Addresses by Decibel Level", x = "Street Address", y = "Capacity") +
  theme_minimal()+
  theme(axis.text.y = element_text(size = 5, margin = margin(r = 5))) +  # Increases spacing in y-axis labels
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.7)))  # Adds space above/below bars

print(p3)

top_decibel_counts <- clean_data %>%
  arrange(desc(DECIBEL_LEVEL)) %>%
  slice_head(n = N) %>%
  count(STREET_ADDRESS, sort = TRUE)

p4 <- ggplot(top_decibel_counts, aes(x = reorder(STREET_ADDRESS, n), y = n)) +
  geom_col(fill = "steelblue") +
  coord_flip() +  # Rotate for readability
  labs(title = paste("Street Addresses with most venues in Top", N, "\n for Decibel Levels"),
       x = "Street Address", y = "Count") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 5, margin = margin(r = 5))) +  # Increases spacing in y-axis labels
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.7)))  # Adds space above/below bars
...
clean_data <- df %>% filter(!is.na(START_HOURS_NEAR_HOME_SUN_THUR) & !is.na(START_HOURS_NEAR_HOME_FRI_SAT))

hour_counts <- clean_data %>%
  pivot_longer(cols = c(START_HOURS_NEAR_HOME_SUN_THUR, START_HOURS_NEAR_HOME_FRI_SAT),
               names_to = "DAY_GROUP",
               values_to = "START_HOUR") %>%
  group_by(DAY_GROUP, START_HOUR) %>%
  tally() %>%
  ungroup()

hour_counts$DAY_GROUP <- factor(hour_counts$DAY_GROUP, levels = c("START_HOURS_NEAR_HOME_SUN_THUR", "START_HOURS_NEAR_HOME_FRI_SAT"), labels = c("Sun-Thurs", "Fri-Sat"))



p5 <- ggplot(hour_counts, aes(x = as.factor(START_HOUR), y = n, fill = DAY_GROUP)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Start Hour Counts: Sun-Thurs vs. Fri-Sat", x = "Start Hour", y = "Count") +
  theme_minimal() +
...
```
# Eshi's Exploratory Graphing
```{r}
# ESHI'S VISUALIZATIONS
# Start to end time durations for concrete pouring
library(ggplot2)  # Load the package
df$START_HOURS_CONC_POUR_MON_SAT <- ifelse(df$START_HOURS_CONC_POUR_MON_SAT == -100, NA, df$START_HOURS_CONC_POUR_MON_SAT)
df$END_HOURS_CONC_POUR_MON_SAT <- ifelse(df$END_HOURS_CONC_POUR_MON_SAT == -100, NA, df$END_HOURS_CONC_POUR_MON_SAT)

df_conc_start <- df %>% filter(!is.na(START_HOURS_CONC_POUR_MON_SAT))
df_conc_end <- df %>% filter(!is.na(END_HOURS_CONC_POUR_MON_SAT))


# Box plot
ggplot(df_conc_start) + 
  geom_boxplot(aes(x = factor(1), y = START_HOURS_CONC_POUR_MON_SAT), fill = "blue", alpha = 0.6) + 
  geom_boxplot(aes(x = factor(2), y = END_HOURS_CONC_POUR_MON_SAT), fill = "red", alpha = 0.6) + 
  scale_x_discrete(labels = c("Start", "End")) + 
...
# Bar plot of start times using a gradient color scale 
'''
ggplot(df_conc_start, aes(x = factor(START_HOURS_CONC_POUR_MON_SAT), fill = START_HOURS_CONC_POUR_MON_SAT)) +
  geom_bar(stat = "count", na.rm = TRUE) +
  scale_fill_gradient2(low = "darkblue", mid = "yellow", high = "darkblue", midpoint = 12) +
  theme_minimal() +
...
# chatGPT code to have start times include the numbers that had a count of 0

full_hours <- data.frame(START_HOURS_CONC_POUR_MON_SAT = 0:23) # Create a full range of hours (0-23)
hour_counts <- df_conc_start %>%
  count(START_HOURS_CONC_POUR_MON_SAT)# Count occurrences of each hour in the dataset

df_full <- full_hours %>%
  left_join(hour_counts, by = "START_HOURS_CONC_POUR_MON_SAT") %>%
  mutate(n = ifelse(is.na(n), 0, n))  # Merge with full range, replacing missing counts with 0 - Replace NAs with 0


ggplot(df_full, aes(x = factor(START_HOURS_CONC_POUR_MON_SAT), y = n, fill = START_HOURS_CONC_POUR_MON_SAT)) +
  geom_bar(stat = "identity", na.rm = TRUE) +  
  scale_fill_gradient2(low = "darkblue", mid = "yellow", high = "darkblue", midpoint = 12) +
  scale_y_continuous(trans = "sqrt", limits = c(0, max(df_full$n) * 1.1), expand = expansion(mult = c(0, 0.05))) +
  theme_minimal() +
  labs(x = "Start Hour", y = "Count (sqrt scale)", fill = "Hour of Day")
...
ggplot(df_conc_end, aes(x = factor(END_HOURS_CONC_POUR_MON_SAT), fill = END_HOURS_CONC_POUR_MON_SAT)) +
  geom_bar(stat = "count", na.rm = TRUE) +
  scale_fill_gradient2(low = "darkblue", mid = "yellow", high = "darkblue", midpoint = 12) +
  theme_minimal() +
  labs(x = "End Hour", y = "Count", fill = "Hour of Day")

# cleaning existing zoning
df$EXISTING_ZONING <- ifelse(df$EXISTING_ZONING == "999 - SEE COMMENTS", NA, df$EXISTING_ZONING)
# cleaning decibel level
df_decibel_fixed <- df %>%
  mutate(DECIBEL_LEVEL = case_when(
    DECIBEL_LEVEL == 8595 ~ 85.95,
    TRUE ~ DECIBEL_LEVEL
  ))

df_clean_decibel <- df %>% filter(!is.na(DECIBEL_LEVEL))
df_clean_zone <- df_clean_decibel %>% filter(!is.na(EXISTING_ZONING))

# plottting existing zoning and decibel level
ggplot(df_clean_zone, aes(x = EXISTING_ZONING, y = DECIBEL_LEVEL)) +
  geom_point() + 
  labs(title = "Relationship Between Zones and Decibel Levels",
       x = "Zone",
       y = "Decibel Level") +
...
# plotting existing zoning and sub type
df_clean_type <- df %>% filter(!is.na(SUB_TYPE))
df_zone_clean <- df_clean_type %>% filter(!is.na(EXISTING_ZONING))
ggplot(df_zone_clean, aes(x = EXISTING_ZONING, fill = SUB_TYPE)) +
  geom_bar(position = "dodge") +
  labs(title = "Distribution of Sub Types Across Zoning Areas",
       x = "Zone",
       y = "Count") +
  theme_minimal() +
...
df_type_clean <- df_clean_decibel %>% filter(!is.na(SUB_TYPE))

average_decibels <- df_type_clean %>%
  group_by(SUB_TYPE) %>%
  summarise(Average_DECIBEL = mean(DECIBEL_LEVEL, na.rm = TRUE), .groups = 'drop')

ggplot(df_type_clean, aes(x = SUB_TYPE, y = DECIBEL_LEVEL)) +
  geom_point(color = "blue") + # Use geom_point with some transparency
  geom_point(data = average_decibels, aes(x = SUB_TYPE, y = Average_DECIBEL), color = "red", size = 3, shape = 18) +
  labs(title = "Decibel Levels by SubType",
       x = "SubType",
       y = "Decibel Level") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
# attempting to combine the zones vs decibel level
# library(tidyverse)
# df_separated <- df_clean_zone %>%
#   mutate(EXISTING_ZONING = strsplit(as.character(EXISTING_ZONING), "-")) %>%
#   unnest(EXISTING_ZONING)

# zone_counts <- df_separated %>%
#   group_by(EXISTING_ZONING, DECIBEL_LEVEL) %>%
#   summarise(Count = n(), .groups = 'drop')

# ggplot(zone_counts, aes(x = DECIBEL_LEVEL, y = Count, fill = EXISTING_ZONING)) +
#   geom_col(position = "dodge") +
#   labs(title = "Frequency of Zoning Types Across Decibel Levels",
#        x = "Decibel Level",
#        y = "Frequency") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
df_times %>% glimpse()
write.csv(df_times, "df_times_cleaned.csv", row.names = FALSE)

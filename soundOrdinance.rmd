...
head(df$START_HOURS_NOT_NEAR_HOME,100)
head(vec,100)
...
test <- df %>%
  mutate(START_HOURS_NOT_NEAR_HOME = sapply(df$START_HOURS_NOT_NEAR_HOME, convert_time))
test %>% filter(!is.na(START_HOURS_NOT_NEAR_HOME)) %>% select(START_HOURS_NOT_NEAR_HOME)
df$START_HOURS_NOT_NEAR_HOME
...
unique(test$START_HOURS_NOT_NEAR_HOME)

...
df$START_HOURS_NOT_NEAR_HOME <- ifelse(df$START_HOURS_NOT_NEAR_HOME == 'Pending Review', NA, df$START_HOURS_NOT_NEAR_HOME) # replaced that pending review with a NA value
df$START_HOURS_NOT_NEAR_HOME <- gsub("an", "am", df$START_HOURS_NOT_NEAR_HOME) # fixed a typo
df$START_HOURS_NOT_NEAR_HOME <- ifelse(df$START_HOURS_NOT_NEAR_HOME == '12:00', "12:00 PM", df$START_HOURS_NOT_NEAR_HOME)# changed 12:00 to 12:00PM
...
table(df$REST_51_PERCENT_FOOD_SALES)



unique(df$REST_51_PERCENT_FOOD_SALES)
num_nas <- sum(is.na(df$REST_51_PERCENT_FOOD_SALES))
print(paste("Number of NAs:", num_nas, nrow(df))) # missing vals??

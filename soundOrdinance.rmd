...
ggplot(avg_dec_zip, aes(x=ZIPCODE, y=avg_dec)) + geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7))
...
#df_times <- df_times %>% filter(DECIBEL_LEVEL < 8000) 
# ggplot(df_times, aes(x=CAPACITY, y=DECIBEL_LEVEL) ) + geom_point() + xlim(0, 5000) +  ylim(0, 120)
...
ggplot(df_cap, aes(x = CAPACITY, y = time_diff)) + 

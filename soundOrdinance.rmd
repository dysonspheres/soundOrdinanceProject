...
ggplot(df_full, aes(x = factor(START_HOURS_CONC_POUR_MON_SAT), y = n, fill = START_HOURS_CONC_POUR_MON_SAT)) +
  geom_bar(stat = "identity", na.rm = TRUE) +  # Use "identity" since we already have counts
  scale_fill_gradient2(low = "darkblue", mid = "yellow", high = "darkblue", midpoint = 12) +
  theme_minimal() +
  labs(x = "Start Hour", y = "Count", fill = "Hour of Day")

# Plot with the corrected dataset


# Bar plot with manually defined colors
ggplot(df_conc_start, aes(x = START_HOURS_CONC_POUR_MON_SAT, fill = START_HOURS_CONC_POUR_MON_SAT)) +
  geom_bar(stat = "count", na.rm = TRUE) +
  scale_fill_manual(values = custom_colors) +  # Apply custom gradient colors
  theme_minimal() +
  labs(x = "Start Hour", y = "Count", fill = "Hour of Day")

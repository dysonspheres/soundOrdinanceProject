...
convert_time <- function(time) {
  if (is.na(time)) { #returns null values
    return(NA)
  }
...
test <- df %>% mutate(START_HOURS_NOT_NEAR_HOME = if_else(!is.na(START_HOURS_NOT_NEAR_HOME),sapply(START_HOURS_NOT_NEAR_HOME, convert_time), NA))
table(test$START_HOURS_NOT_NEAR_HOME)



df$START_HOURS_NOT_NEAR_HOME <- sapply(df$START_HOURS_NOT_NEAR_HOME, convert_time)
table(df$START_HOURS_NOT_NEAR_HOME)


unique(df$START_HOURS_NOT_NEAR_HOME)
df %>% filter(is.na(START_HOURS_NOT_NEAR_HOME)) %>% summarize(n())
df %>% filter(START_HOURS_NOT_NEAR_HOME == 'Pending Review')# %>% select(EXPIRATION_DATE, START_DATE, STATUS) #  %>% summarize(n())  #checking cols of null start_date
head(df, 1)

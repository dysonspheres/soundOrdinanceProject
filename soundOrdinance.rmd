...
ggplot(df_times, aes(x=ZIPCODE)) + geom_bar(stat = 'count')

# time to get permit by capacity
# i am going to TO_LOWERCASE capacity and make see approved load cards --> NA
# then line graph or scatter plot with capacity and time it takes to get approved

# removing anything that is not a number for this section
# NO CORRELATION - so this graph is lowkey useless
df_cap <- df_times %>% mutate(CAPACITY = tolower(CAPACITY)) %>% 
...
df_cap_NO_OUTLIERS <- df_cap %>% filter(CAPACITY < 10000)
...
# should i make one for start dates? 

# df_times %>% glimpse()
# unique(df_times$DECIBEL_LEVEL)
